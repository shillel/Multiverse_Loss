...
model:add(nn.SpatialAveragePooling(8,8,1,1):ceil())
local repSize = 192 --* 8 * 8 --192

model:add(nn.View(repSize))


--model:add(nn.Linear(32 * 8 * 8, repSize))
--w, dE_dw = model:getParameters()
local b1 = nn.Sequential()
local b2 = nn.Sequential()
local b3 = nn.Sequential()
b1:add(nn.Linear(repSize,10))
b2:add(nn.Linear(repSize,10))
b3:add(nn.Linear(repSize,10))
...
criterion = nn.ParallelCriterion():add(crit1):add(crit2):add(crit3):add(crito1,10):add(crito2,10):add(crito3,10)
...
function forwardNet(data,labels, train)
    ...
    for i = 1, data:size(1) - batchSize, batchSize do
        ...
        local A = torch.CudaTensor(batchSize,1):fill(-1)
        local err  = criterion:forward(y, {yt,yt,yt,A,A,A})
        ...
        if train then
            function feval()
                ...
                local dE_dy = criterion:backward(y, {yt,yt,yt,A,A,A})
...
function plotError(trainError, testError, title)
	...
	gnuplot.pngfigure('testVsTrainError_3v.png')
...
saveTensorAsGrid(testData[10],'testImg10_3v.jpg')
